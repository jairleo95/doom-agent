...
if [ -z "$TARGET_PATH" ]; then
    ...
    LATEST_RUN=$(ls -td src/doom_agent/algorithms/dreamer_v3/checkpoints/deathmatch_curriculum/*/ 2>/dev/null | head -1)
    if [ -z "$LATEST_RUN" ]; then
        echo "Error: No recent deathmatch_curriculum runs found in checkpoints."
        echo "Usage: ./watch_dreamer.sh src/doom_agent/algorithms/dreamer_v3/checkpoints/deathmatch_curriculum/RUN_ID"
        exit 1
    fi
...
python src/doom_agent/algorithms/dreamer_v3/visualize.py \
    --path "$TARGET_PATH" \
