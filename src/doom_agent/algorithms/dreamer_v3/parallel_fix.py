...
class Parallel:
    def __init__(self, ctor, strategy):
        self.worker = Worker(bind(_respond, ctor), strategy, state=True)
        self.callables = {}
...
def _respond(ctor, state, message, name, *args, **kwargs):
    if state is None:
        try:
            ...
            print(f"DEBUG Parallel: Error calling ctor: {e}")
            raise
        
        # Double check if state is actually the environment
        if hasattr(state, '__call__') and not hasattr(state, 'reset'):
            try:
                state = state()
            except Exception:
                pass
