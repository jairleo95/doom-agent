...
from res.DeepQLearner.models.perceptron import SLP
from res.DeepQLearner.models.cnn import CNN

from res.DeepQLearner.utils.decay_schedule import LinearDecaySchedule
from res.DeepQLearner.utils.experience_memory import ExperienceMemory, Experience
from res.DeepQLearner.utils.params_manager import ParamsManager

import res.DeepQLearner.environments.utils as env_utils
...
# habilitar por gpu o cpu
#import tensorflow as tf
#print("Num GPUs Available: ", len(tf.config.experimental.list_physical_devices('GPU')))
print("Cuda is avilable:"+ str(torch.cuda.is_available()))
...
class DeepQLearner(object):
    ...
    def __init__(self, obs_shape, action_shape, params):
        ...
        if self.params['use_target_network']:  # actualizar los parametros con los ultimos numeros (actualizacion de la experiencia)
    ...
    def get_action(self, obs):
        obs = np.array(obs)
        obs = obs / 255.0
        #print("get_action().obs.shape: " + str(obs.shape))
        if len(obs.shape) == 3:  # tenemos una imagen
            if obs.shape[2] < obs.shape[0]:  # alturaxAnchuraxColor
    ...
    def epsilon_greedy_Q(self, obs):
        ...
        self.step_num += 1  # para saber en que iteracio voy
    ...
    def replay_experience(self, batch_size=None):
        ...
        self.training_steps_completed += 1  # contador para ver el numero de entrenamientos completados
    ...
    def learn_from_batch_experience(self, experiences):
        ...
        obs_batch = np.array(batch_exp.obs) / 255.0  # escalar de 0 a 1
        action_batch = np.array(batch_exp.action)
        reward_batch = np.array(batch_exp.reward)

        if self.params["clip_reward"]:
            reward_batch = np.sign(reward_batch)  # devolver el signo
        ...
        if self.params['use_target_network']:
            ...
            td_target = reward_batch + ~done_batch * \
                        ...
                        self.Q(next_obs_batch).detach().max(1)[
                            0].data.cpu().numpy()  # convertimos a numpy array para multiplicar y se revierte con torch.from_numpy
...
if __name__ == "__main__":
    ...
    print("obs_shape: "+str(obs_shape)+", action_shape: "+str(action_shape)+", agent_params:"+str(agent_params))
    ...
    while global_step_num < agent_params["max_training_steps"]:
        ...
        while not done:
            ...
            #print("\033[94m action:"+str(action))
            agent.memory.store(Experience(obs, action, reward, next_obs, done))
            print("memory size: "+ str(agent.memory.get_size()))
            ...
            if done:
                ...
                print("\n Episodio #{} finalizado con {} iteraciones. Con {} estados ={}, recompensa media = {:2}, mejor recompensa ={})".
